...
public class CombinedMesh : MonoBehaviour
{
    ...
    public void Combine(CombineProgressDelegate progress)
    {
        ...
        //float maxZ = 0, minZ = 0;
        //bool isFirst = true;
        //foreach (Transform item in RootNode.GetComponentsInChildren<Transform>(true))
        //{
        //    if (item.Equals(RootNode.transform))
        //        continue;
        //    if (isFirst)
        //    {
        //        maxX = item.transform.position.x;
        //        minX = item.transform.position.x;
        //        maxY = item.transform.position.y;
        //        minY = item.transform.position.y;
        ...
        //    {
        //        //最大值
        //        maxX = maxX > item.transform.position.x ? maxX : item.transform.position.x;
        //        maxY = maxY > item.transform.position.y ? maxY : item.transform.position.y;
        //        maxZ = maxZ > item.transform.position.z ? maxZ : item.transform.position.z;
        //        //最小值
        //        minX = minX < item.transform.position.x ? minX : item.transform.position.x;
        //        minY = minY < item.transform.position.y ? minY : item.transform.position.y;
        //        minZ = minZ < item.transform.position.z ? minZ : item.transform.position.z;
        //    }
        //}
        //float PosX = (maxX + minX) / 2;
        //float PosY = (maxY + minY) / 2;
        //float PosZ = (maxZ + minZ) / 2;
    ...
    private void Reset()
    {
        ...
        // BuildMeshFilterListRecursive(RootNode, listMeshFilters);
        //MeshObjects = listMeshFilters.ToArray();
        // Combine(null);
    }
